...
export default function App() {
  ...
  return (
    <div className={`min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-gray-800 dark:text-gray-100 flex flex-col justify-between ${currentLang === 'ar' ? 'rtl' : 'ltr'}`} dir={currentLang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      {/* 3. STICKY NAVBAR MAIN HEADER WITH CRISP CONTRAST FOR LIGHT & DARK MODE */}
      <header
        id="sticky-header"
        className={`sticky top-0 z-50 w-full h-[60px] px-3 sm:px-6 flex items-center transition-all duration-300 ${
          isDarkMode
      ...
      >
        ...
        {isMobileMenuOpen && (
          <div className={`lg:hidden w-full mt-3 pt-3 border-t animate-in fade-in slide-in-from-top-4 duration-200 ${
            isHeroNavbar ? 'border-emerald-800/50 text-white' : 'border-gray-100 dark:border-gray-800'
          }`}>
            {currentView === 'admin' ? (
              ...
              <div className="flex flex-col gap-1.5">
                {[
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3.5 py-2 text-xs transition-all cursor-pointer bg-transparent border-none ${
                        isActive
                          ? 'font-black text-emerald-300 [text-shadow:0_0_8px_#34d399,0_0_16px_#10b981]'
                          : 'font-semibold text-gray-700 dark:text-gray-300 hover:text-emerald-500'
                      }`}
                    >
                      {langPack[item.key] || t(item.label)}
                    </button>
                ...
                <div className={`border-t my-1 pt-1 ${isHeroNavbar ? 'border-emerald-800/40' : 'border-gray-100 dark:border-gray-800'}`}>
                  ...
                  <div className="grid grid-cols-2 gap-1.5 mt-1">
                    {[
                      ...
                      <button
                        ...
                        className={`text-left px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-colors cursor-pointer ${
                          activeSection === subItem.id
                            ? 'text-emerald-300 font-extrabold [text-shadow:0_0_8px_#34d399] bg-emerald-950/50'
                            : isHeroNavbar
                            ? 'text-slate-200 hover:bg-white/10 hover:text-white'
                            : 'text-gray-600 dark:text-gray-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 hover:text-emerald-700 dark:hover:text-emerald-400'
                        }`}
                      >
                  ...
                  <div className={`flex gap-2 pt-2 mt-2 border-t ${isHeroNavbar ? 'border-emerald-800/40' : 'border-gray-100 dark:border-gray-800'}`}>
                    <button
                      onClick={() => {
                        setIsMobileMenuOpen(false);
                        setIsFeedbackOpen(true);
                    ...
                    {currentAdmin && (
                      <button
                        ...
                        className="flex-1 text-center py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5"
                      >
      ...
      {currentView !== 'admin' && (
        <>
          ...
          <button
            ...
            className="fixed bottom-6 left-6 z-40 bg-slate-900/90 hover:bg-slate-900 dark:bg-emerald-850 dark:hover:bg-emerald-750 border border-slate-700/80 p-3.5 rounded-full text-white shadow-2xl hover:scale-105 transition-all flex items-center justify-center gap-2 cursor-pointer group"
            title={t('Kirim Masukan & Evaluasi Tools')}
          >
            <MessageSquarePlus className="w-5 h-5 text-emerald-400 group-hover:rotate-12 transition-transform" />
            <span className="text-xs font-black hidden sm:inline-block pr-1">{t('Feedback')}</span>
          </button>
